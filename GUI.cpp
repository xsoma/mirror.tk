...
void CGUI::Update()
{
	...
	for (auto window : Windows)
	{
		if (window->m_bIsOpen)
		{
			...
			Render::Clear(0, 0, w * 4, h * 4, Color(9, 9, 9, (MenuAlpha / (1.26 + owo))));
		...
		else
		{
			...
			//	Render::Text((centerW * 2) - 400, 35, Color(250, 250, 250, (MenuAlpha - 1)), Render::Fonts::xd, "Mirror v6 by FreaK");
				Render::Textf((centerW * 2) - 400, 15, Color(240, 240, 240, (MenuAlpha - 1)), Render::Fonts::xd,("Mirror | %s | fps: %d "), GetTimeString().c_str(), getfps());
