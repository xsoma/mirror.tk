...
void antiaim_helper::anti_lby(CUserCmd* cmd, bool& bSendPacket)
{
	...
	if (game_utils::IsBomb(pWeapon) || game_utils::IsGrenade(pWeapon))
		return;
	...
	else if (shouldBreak)
	{
		...
		case 1:
		{
			...
			brokeThisTick = true;
			oldCurtime = interfaces::globals->curtime;
			cmd->viewangles.y = c_beam->real + options::menu.MiscTab.BreakLBYDelta.GetValue();
			shouldBreak = false;
		}
		break;
		...
		case 3:
		{
			float addAngle = GetFPS() >= (TIME_TO_TICKS(1.f) * 0.5f) ? (2.9 * max(choked, GlobalBREAK::prevChoked) + 100) : 149.0f;

			if (bSendPacket)
			{
				brokeThisTick = true;
				oldCurtime = interfaces::globals->curtime;
				cmd->viewangles.y += addAngle - 29.f;		
			}
			else
			{
				brokeThisTick = true;
