...
void Render::Initialise()
{
	...
	interfaces::surface->SetFontGlyphSet(Fonts::nnbruda, "Smallest Pixel-7", 19, 130, 0, 0, FONTFLAG_OUTLINE);
...
void Render::DrawRectRainbow(int x, int y, int width, int height, float flSpeed, float &flRainbow,float alpha)
{
	...
	for (int i = 0; i < width; i++)
	{
		...
		Color colRainbow = colColor.FromHSB(hue / 2, 0.1f, hue);
