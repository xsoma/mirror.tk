...
void CComboBoxYeti::Draw(bool hover)
{
	...
	Render::gradient_verticle(a.x, a.y, m_iWidth, 20, Color(35, 35, 35, MenuAlpha), Color(35, 35, 35, MenuAlpha));
	...
	if (Items.size() > 0)
	{
		...
		if (IsOpen)
		{
			Render::gradient_verticle(a.x, a.y + 20, m_iWidth, Items.size() * 20, Color(35, 35, 35, MenuAlpha), Color(35, 35, 35, MenuAlpha));
			...
			for (int i = 0; i < Items.size(); i++)
			{
				...
				if (GUI.IsMouseInRegion(ItemRegion))
				{
					Render::gradient_verticle(a.x, a.y + 20 + i * 20, m_iWidth, 20, Color(25, 25, 25, MenuAlpha), Color(25, 25, 25, MenuAlpha));
				}
				...
				if (i == SelectedIndex)
					...
					Render::text_yeti(a.x + 10, a.y + 20 + (i * 20) + 10 - (control_textsize.bottom / 2), Items[i].c_str(), Render::Fonts::MenuText, Color(140, 140, 140, MenuAlpha));
			}
